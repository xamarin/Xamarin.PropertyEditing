...
{
	internal class DirectoryPathEditorControl : PropertyEditorControl<PropertyViewModel<DirectoryPath>>
	{
		const string SelectDirectoryTitle = "Choose a Directory";
		...
		void HandleAction (System.nint result)
		{
			if (result == 1) {
				var url = panel.Url;
				if (url.IsFileUrl) {
					ViewModel.Value = new DirectoryPath (url.Path);
		...
		protected override void UpdateValue ()
		{
			if (ViewModel.Value == null) {
				...
				ButtonEditor.Title = string.Format ("{0}...", SelectDirectoryTitle);
			} else {
